...
namespace M2MqttExampleClient
{
        ...
        private IEnumerable<ExternalSensorMetadata> ReadAllDataSensors()
        {
            ...
            return _reader.GetRecords<TransmittedDataSensor>()
                .Select(e =>
                {
                    ...
                    mapping.VectorElementsDatas.Add(new ExternalVectorElementMetadata
                    {
    ...
    {
        public static RemoteSourceAvailableSensors GetAvailableSensors(string sourceId, EventType eventType, string sensorListFilePath)
        {
            try
            {
                    ...
                    switch (eventType)
                    {
                        ...
                        case EventType.StateChangeEventType:
                            ...
                            foreach (var sensor in sensors)
                            {
                                ...
                                sensorMetadata.Properties.Add("DisplayName", new Property(sensor.TargetNodeDisplayName));
                        ...
                        case EventType.SensorDataEventType:
                            foreach (var sensor in sensors)
                            {
                                ...
                                {
                                    Name = string.IsNullOrEmpty(sensor.VectorElementsDatas.First().Name) ? sensor.VectorElementsDatas.First().Name : sensor.VectorElementsDatas.First().Description,
                                    SourceUnit = sensor.VectorElementsDatas.First().MeasureUnit
                                };
                                sensorMetadata.VectorElementsDatas.Add(vector);
                                sensorMetadata.Properties.Add("DisplayName", new Property(sensor.TargetNodeDisplayName));
                                availableSensors.Sensors.Add(sensorMetadata);
                            }
                            break;
